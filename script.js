/**
 * PulseTime Pro - Core JavaScript (v1.2)
 * Refined for Professional UI & Hamburger Menu
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const playAlarm = () => {
        ...
        if (audio) {
            audio.currentTime = 0;
            audio.play().catch(() => showToast('Timer Finished!', 'success'));
        }
    };

    // --- Navigation & Menu Logic ---
    const toggleMenu = (show) => {
        const isVisible = show !== undefined ? show : !sidebar.classList.contains('open');
        sidebar.classList.toggle('open', isVisible);
        menuOverlay.classList.toggle('show', isVisible);
        menuToggle.innerHTML = isVisible ? '<i class="fa-solid fa-xmark"></i>' : '<i class="fa-solid fa-bars"></i>';
    };
    ...
    const switchTab = (tabId) => {
        ...
        toggleMenu(false); // Close menu on mobile
    ...
    tStartBtn.addEventListener('click', () => {
        if (state.timer.isRunning) {
            ...
            if (state.timer.remaining > 0) {
                ...
                showToast('Please set a time', 'info');
            }
        }
    });
    ...
    const updatePomo = () => {
        ...
        if (diff <= 0) {
            ...
            playAlarm();
            showToast(`${state.pomodoro.mode.toUpperCase()} finished!`, 'success');
        }
    };
    ...
    const updateSleep = () => {
        ...
        if (diff <= 60 && diff > 0) fadeStatus.textContent = `Fading audio... ${Math.round((diff/60)*100)}%`;
        if (diff <= 0) {
            clearInterval(state.sleep.interval);
            state.sleep.isRunning = false;
            sleepStartBtn.textContent = 'Start';
            fadeStatus.textContent = 'Complete.';
    ...
    const renderMultiTimers = () => {
        ...
        if (state.multiTimers.length === 0) {
            multiContainer.innerHTML = '<div class="glass-card" style="grid-column: 1/-1; text-align: center; color: var(--text-muted);">No timers. Tap "New Timer".</div>';
            return;
        }
    ...
    const renderAlarms = () => {
        ...
        if (state.alarms.length === 0) {
            alarmContainer.innerHTML = '<div class="glass-card" style="text-align: center; color: var(--text-muted);">No alarms.</div>';
            return;
        }
    ...
    alarmContainer.addEventListener('click', (e) => {
        ...
        if (btn) {
            state.alarms.splice(parseInt(btn.getAttribute('data-index')), 1);
            renderAlarms();
